...
class Workspace:
    ...
    def create_adroit_env(self):
        ...
        self.train_env = dhm.make_env_RRL_sam(self.env_name, test_image=False, num_repeats=self.cfg.action_repeat, input_resolution=self.cfg.input_resolution,
    ...
    def setup(self):
        ...
        if env_type == 'adroit':
            self.train_env = dhm.make_env_RRL_sam(env_name, test_image=False, num_repeats=self.cfg.action_repeat, input_resolution=self.cfg.input_resolution,
                                              num_frames=self.cfg.frame_stack, env_feature_type=self.env_feature_type, use_SAM_g=self.cfg.use_SAM_g, handmade_4_SAM=self.cfg.handmade_4_SAM,
                                              device=self.device, reward_rescale=self.cfg.reward_rescale, seed=self.cfg.seed)
            self.eval_env = dhm.make_env_RRL_sam(env_name, test_image=False, num_repeats=self.cfg.action_repeat, input_resolution=self.cfg.input_resolution,
                                             num_frames=self.cfg.frame_stack, env_feature_type=self.env_feature_type, use_SAM_g=self.cfg.use_SAM_g, handmade_4_SAM=self.cfg.handmade_4_SAM,
                                             device=self.device, reward_rescale=self.cfg.reward_rescale, seed=self.cfg.seed)
    ...
    def eval_dmc(self):
        ...
        if self.cfg.use_wandb:
            wandb.log({"eval_dmc_reward": total_reward/episode})
    ...
    def load_demo(self, replay_storage, env_name, verbose=True):
        ...
        demo_env = dhm.make_env_RRL_sam(env_name, test_image=False, num_repeats=1, num_frames=self.cfg.frame_stack, input_resolution=self.cfg.input_resolution,
    ...
    def train(self):
        ...
        while train_until_step(self.global_step):
            ...
            if self.global_step % 1000 == 0 and metrics is not None:
                ...
                if self.cfg.use_wandb:
                    wandb.log({"train_reward": np.mean(episode_reward_list)})
...
@hydra.main(config_path='cfgs_adroit', config_name='pieg_config')
def main(cfg):
    W = Workspace
    root_dir = Path.cwd()
