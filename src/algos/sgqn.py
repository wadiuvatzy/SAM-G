...
class AttributionDecoder(nn.Module):
    def __init__(self,action_shape, emb_dim=100, input_resolution=84) -> None:
        super().__init__()
        self.proj = nn.Linear(in_features=emb_dim+action_shape, out_features=32*(input_resolution//4)*(input_resolution//4))
...
class AttributionPredictor(nn.Module):
    def __init__(self, action_shape, encoder, input_resolution=84):
        super().__init__()
        self.encoder = encoder
        self.decoder = AttributionDecoder(action_shape, encoder.repr_dim, input_resolution=input_resolution)
...
class SGQNAgent(VRL3Agent):
    ...
    def __init__(self, aux_lr=0.3, aux_beta=0.9, sgqn_quantile=0.95, **kwargs):
        ...
        self.attribution_predictor = AttributionPredictor(kwargs['action_shape'][0],
                                                          self.encoder, input_resolution=kwargs['obs_shape'][1]).to(self.device)
