...
class Workspace:
    def __init__(self, cfg):
        self.work_dir = '../../' + cfg.task_name + '/' + str(cfg.seed)
        ...
        self.agent_name = 'vrl3'
        self.cfg.task_name = self.cfg.task_name.split('-')[0]
        work_dir = '../../' + cfg.task_name+'-v0' + '/' + str(cfg.seed)
    ...
    def save_demo(self, force_number_episodes=None, do_log=True):
        step, episode, total_reward = 0, 0, 0
        n_eval_episode = 300
        total_success = 0.0
        for i in tqdm(range(n_eval_episode)):
            demo_save_path = '../../demos/' + self.cfg.task_name + '/' + str(i) + '.pkl'
            demos = []
                ...
                if self.agent_name == 'pieg':
                    with torch.no_grad():
                        ...
                        s = {"observation": observation,
                             "action": action,
                             "sensor": time_step.observation_sensor,}
                        demos.append(s)
