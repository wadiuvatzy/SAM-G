...
class Trainer:
    ...
    def _validate(self, model, data_loader, epoch) -> dict:
        raise NotImplementedError

    def validate(self, model=None, data_loader=None, is_test=True, epoch=0) -> dict:
    ...
    def before_step(self, feed_dict: dict) -> dict:
        for key in feed_dict:
            if isinstance(feed_dict[key], torch.Tensor):
                feed_dict[key] = feed_dict[key].cuda()
        return feed_dict

    def run_step(self, feed_dict: dict) -> dict:
        raise NotImplementedError
