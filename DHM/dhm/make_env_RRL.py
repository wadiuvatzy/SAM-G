"""
Two masked input to get points prompt, only generate one mask
"""
import os
import gc
...
import sys
sys.path.append('/home/ziyuwang21/workspace/PerSAM/Personalize-SAM-main')
import per_segment_anything as psa
import persam_f as psf
import pdb
sys.path.append("/home/ziyuwang21/workspace/EfficientVit/efficientvit")
...
class BasicDHMEnv(gym.Env, ABC):
    def __init__(self, env, feature_type, cfg_dict, cameras, latent_dim=512, hybrid_state=True, channels_first=False,
        ...
        #    name='l1', weight_url="/home/ziyuwang21/workspace/EfficientVit/efficientvit/checkpoints/l1.pt"
        # )
        # efficientvit_sam = efficientvit_sam.cuda().eval()
        # self.efficientvit_sam_predictor = EfficientViTSamPredictor(efficientvit_sam)

        # specific_image = '/home/ziyuwang21/workspace/vigen_adroit/door_0.png'
        # specific_masked = '/home/ziyuwang21/workspace/vigen_adroit/door_masked_0.jpg'
        
        # target_feat_list = []
        # ref_image = cv2.imread(specific_image)
        # ref_image = cv2.cvtColor(ref_image, cv2.COLOR_BGR2RGB)

        # ref_mask = cv2.imread(specific_masked)
        # ref_mask = cv2.cvtColor(ref_mask, cv2.COLOR_BGR2RGB)

        # cv2.imwrite("/home/ziyuwang21/workspace/vigen_adroit/hello.png", ref_image)
        ...
        # # ref_mask = self.efficientvit_sam_predictor.get_processed_mask(ref_mask)
        # # self.efficientvit_sam_predictor.set_image(ref_image)
        ...
        # parts_target_feat = []
        # extra_points = [[38, 14], [42, 31], [28,34],[36,59], [38,61], [37, 75]] # for door_0
        ...
        # for i in range(2):
        #    ref_mask = cv2.imread('/home/ziyuwang21/workspace/vigen_adroit/door_masked_0_{}.jpg'.format(i+1))
        #    ref_mask = cv2.cvtColor(ref_mask, cv2.COLOR_BGR2RGB)
        ...
        #    # plt.savefig('/home/ziyuwang21/workspace/vigen_adroit/sim.png')
        #    # plt.close()

        #    # plt.imshow(ref_image)
        #    # for k in range(len(topk_xy_)):
        #    #  if topk_label_[k] == 1:
        #    #   plt.scatter(topk_xy_[k][0], topk_xy_[k][1], c='r')
        #    #  else:
        #    #   plt.scatter(topk_xy_[k][0], topk_xy_[k][1], c='b')
        #    # plt.savefig('/home/ziyuwang21/workspace/vigen_adroit/topk.png')
        #    # plt.close()
        #    # input("showing the similarity given two specific image")
        ...
        #    for part_feat in parts_target_feat:
        ...
        #    #  if label[k] == 1:
        #    #   plt.scatter(xy[k][0], xy[k][1], c='r')
        #    #  else:
        #    #   plt.scatter(xy[k][0], xy[k][1], c='b')
        #    # plt.savefig('/home/ziyuwang21/workspace/vigen_adroit/topk.png')
        #    # plt.close()
        #    # input("showing prompts for training 3 weights")
    ...
    def get_obs(self):
        ...
        # for target_feat in self.part_target_feat:
        ...
        #       original_size=self.efficientvit_sam_predictor.original_size,
        #    ).squeeze()
        #    xy, label = psf.point_selection(sim, topk=1)
        ...
        # # for i in range(len(topk_xy)):
        # #  if topk_label[i] == 1:
        # #   plt.scatter(topk_xy[i][0], topk_xy[i][1], c='r')
        # #  else:
        # #   plt.scatter(topk_xy[i][0], topk_xy[i][1], c='b')
        # # plt.gca().add_patch(plt.Rectangle((input_box[0], input_box[1]), input_box[2] - input_box[0], input_box[3] - input_box[1], fill=False, edgecolor='g', linewidth=3))
        # # plt.savefig('/home/ziyuwang21/workspace/vigen_adroit/wrapper_visualization_2.png')
        # # plt.close()
        # # plt.imshow(logit)
        # # plt.savefig('/home/ziyuwang21/workspace/vigen_adroit/wrapper_visualization_2_logit.png')
        # # plt.close()
        ...
        # # cv2.imwrite('/home/ziyuwang21/workspace/vigen_adroit/wrapper_mask.png', obs.transpose(1, 2, 0)[:, :, ::-1])
        # # input("visualizing, press enter to continue...")
...
class DHMEnv():  # dhm is for dexterous hand manipulation
    ...
    def step(self, action, force_step_type=None, debug=False):
        ...
        # print(" time step ", time_step.observation.shape, time_step.observation_sensor.shape)
        # # cv2.imwrite('/home/ziyuwang21/workspace/vigen-adroit/test.png', time_step.observation.transpose(1, 2, 0))
